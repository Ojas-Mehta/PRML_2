...
Q2 = 2; % Number of clusters in class 2
Q3 = 1; % Number of clusters in class 3
Q4 = 2; % Number of clusters in class 4
Q5 = 1; % Number of clusters in class 5
% 
% % 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%class 1 training%%%%%%%%%%%%
X1 = X1_train;
...
%     mattwo=[];
%     for j = 1:size(indices1,1)
%         if(indices1(j)==i)
%           mattwo=[mattwo;X1(j,:)];
%         end
%     end
%     clusterpoints1(i)={mattwo};
...
for i = 1:Q5
    ...
    temp5 = points5- mu5(i, :);
    sigma5(i) = {(1/(Ni5(i)-1))*(temp5' * temp5)};
end
...
while (abs(L_new - L_old)>5)
    ...
    for i = 1:Q5
        ...
        sigma5(i) = { (1/Ni5(i))*((X5-mu5(i, :))' *  diag(gamma5(:, i)) * (X5-mu5(i, :))) };  
...
for p=3:length(Files)
    ...
    prdct(1,1)=prdct(1,1)*prior1;
    prdct(1,2)=prdct(1,2)*prior2;
    prdct(1,3)=prdct(1,3)*prior3;
    prdct(1,4)=prdct(1,4)*prior4;
    prdct(1,5)=prdct(1,5)*prior5;
    
    [dontcare, data_class(i)] = max(prdct);
    predicted1(indx,1)=data_class(i);
    if(data_class(i)==1) count2=count2+1; end
    indx=indx+1;
end
disp('Class 1 over');
...
for p=3:length(Files)
    ...
    if(data_class(i)==2) count2=count2+1; end
    indx=indx+1;
end
disp('Class 2 over');
...
for p=3:length(Files)
    ...
    if(data_class(i)==3) count2=count2+1; end
    indx=indx+1;
end
disp('Class 3 over');
...
for p=3:length(Files)
    ...
    if(data_class(i)==4) count2=count2+1; end
end
disp('Class 4 over');
...
for p=3:length(Files)
    ...
    if(data_class(i)==5) count2=count2+1; end
    indx=indx+1;
end
disp('Class 5 over');
 ...
 classification_accuracy=double(count2)/(N1+N2+N3+N4+N5);
